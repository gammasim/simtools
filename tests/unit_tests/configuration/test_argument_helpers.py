...
from simtools.configuration.arguments import CORSIKA, SHOWER
...
def test_one_or_many_action():
    test_parser = parser.CommandLineParser()
    test_parser.add_argument_definitions((CORSIKA.primary(),))
...
def test_quantity_pair_action():
    test_parser = parser.CommandLineParser()
    test_parser.add_argument_definitions((SHOWER.energy_range(),))
