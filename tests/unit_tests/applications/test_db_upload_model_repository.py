...
def test_add_arguments_includes_repository_dir_option():
    parser = argparse.ArgumentParser()

    app._add_arguments(parser)

    args = parser.parse_args(["--repository_dir", "/tmp/simulation-models"])
    assert args.repository_dir == "/tmp/simulation-models"
...
def test_main_forwards_repository_dir(
    ...
    app_context = SimpleNamespace(
        args={
            ...
            "max_attempts": 3,
            "repository_dir": "/tmp/simulation-models",
        },
        db_config={},
    )
