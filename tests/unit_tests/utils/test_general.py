...
def test_quantity_from_db_parameter():
    test_quantity = 5.0 * u.m

    # value, unit, type
    test_1 = {"value": 5.0, "unit": "m", "type": np.float64}
    quantity_1 = gen.quantity_from_db_parameter(test_1)
    assert quantity_1.unit == test_quantity.unit
    assert quantity_1.value == pytest.approx(test_quantity.value)

    # Value, unit, type
    test_2 = {"Value": 5.0, "unit": "m", "type": np.float64}
    quantity_2 = gen.quantity_from_db_parameter(test_2)
    assert quantity_2.unit == test_quantity.unit
    assert quantity_2.value == pytest.approx(test_quantity.value)

    # Value, units, type
    test_3 = {"Value": 5.0, "units": "m", "type": np.float64}
    assert gen.quantity_from_db_parameter(test_3) == test_quantity
    quantity_3 = gen.quantity_from_db_parameter(test_3)
    assert quantity_3.unit == test_quantity.unit
    assert quantity_3.value == pytest.approx(test_quantity.value)

    # unit given as ""
    test_4 = {"Value": 5.0, "unit": "", "type": np.float64}
    quantity_4 = gen.quantity_from_db_parameter(test_4)
    assert quantity_4.unit == u.Unit(u.dimensionless_unscaled)
    assert quantity_4.value == pytest.approx(5.0)

    # no unit
    test_5 = {"Value": 5.0, "type": np.float64}
    quantity_5 = gen.quantity_from_db_parameter(test_5)
    assert quantity_5.unit == u.Unit(u.dimensionless_unscaled)
    assert quantity_5.value == pytest.approx(5.0)

    test_6 = None
    assert gen.quantity_from_db_parameter(test_6) is None

    test_7 = {"Value": 5, "type": np.int64}
    assert gen.quantity_from_db_parameter(test_7) == 5

    test_8 = {"Value": "test_string", "type": str}
    assert gen.quantity_from_db_parameter(test_8) == "test_string"

    test_9 = {"value": 5.0, "unit": "m", "type": "np.float64"}
    quantity_9 = gen.quantity_from_db_parameter(test_9)
    assert quantity_9.unit == test_quantity.unit
    assert quantity_9.value == pytest.approx(test_quantity.value)
