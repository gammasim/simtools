...
def test_get_file_name(corsika_config, io_handler):
    file_name = "proton_South_test-layout_za020-azm000deg_cone0-5_test-corsika-config"
    ...
    assert corsika_config.get_file_name("output_generic") == (
        "corsika_runXXXXXX_proton_za020deg_azm000deg_South_test-layout_test-corsika-config.zst"
    )
    assert corsika_config.get_file_name("multipipe") == "multi_cta-South-test-layout.cfg"
    with pytest.raises(ValueError):
