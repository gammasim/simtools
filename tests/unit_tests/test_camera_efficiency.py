...
def results_file(db):
    ...
    db.exportFileDB(
        dbName="test-data",
        dest=io.getCameraEfficiencyOutputDirectory(cfg.get("outputLocation"), "validate_camera_efficiency"),
        fileName=testFileName
    )

    return io.getCameraEfficiencyOutputDirectory(cfg.get("outputLocation"), "validate_camera_efficiency").joinpath("camera-efficiency-North-LST-1-za20.0_validate_camera_efficiency.ecsv")
...
def test_calc_camera_efficiency(telescope_model, camera_efficiency, results_file):
    ...
    assert camera_efficiency.calcCameraEfficiency() == pytest.approx(0.24468117923810984)  # Value for Prod5 LST-1


def test_calc_tel_efficiency(telescope_model, camera_efficiency, results_file):
    camera_efficiency._readResults()
    telescope_model.exportModelFiles()
    assert camera_efficiency.calcTelEfficiency() == pytest.approx(0.23988884493787524)  # Value for Prod5 LST-1


def test_calc_tot_efficiency(telescope_model, camera_efficiency, results_file):
    camera_efficiency._readResults()
    telescope_model.exportModelFiles()
    assert camera_efficiency.calcTotEfficiency(camera_efficiency.calcTelEfficiency()) == pytest.approx(0.48018680628175714)  # Value for Prod5 LST-1


def test_calc_reflectivity(camera_efficiency, results_file):
    camera_efficiency._readResults()
    assert camera_efficiency.calcReflectivity() == pytest.approx(0.9167918392938349)  # Value for Prod5 LST-1


def test_calc_nsb_rate(telescope_model, camera_efficiency, results_file):
    camera_efficiency._readResults()
    telescope_model.exportModelFiles()
    assert camera_efficiency.calcNsbRate()[0] == pytest.approx(0.2366432742)  # Value for Prod5 LST-1
