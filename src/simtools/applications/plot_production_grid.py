...
altitude / azimuth coordinates. The input can be a plain production grid JSON file
or the JSON generated by ``simtools-production-derive-statistics``. In the latter
case, the application reads the nested ``grid_point`` entries and ignores derived
statistics when plotting.
