...
def _kernel_plot_2d_photons(histograms_instance, property_name, log_z=False):
    ...
    if property_name not in histograms_instance.dict_2d_distributions:
        msg = (
            f"The property {property_name} does not exist. Valid entries are "
    ...
    for i_hist, _ in enumerate(x_bin_edges):
        ...
        fig.colorbar(mesh)
        all_figs.append(fig)
        ax.set_title(f"{histograms_instance.dict_2d_distributions[property_name]['file name']}")
        plt.close()
...
def _kernel_plot_1d_photons(histograms_instance, property_name, log_y=True):
    ...
    property_name: string
        ...
        "counts", "density", "direction", "time", "altitude", "num_photons_per_event"
    log_y: bool
    ...
    for i_hist, _ in enumerate(bin_edges):
        ...
        if log_y is True:
            ax.set_yscale("log")
        ax.set_title(f"{histograms_instance.dict_1d_distributions[property_name]['file name']}")
        all_figs.append(fig)
        plt.close(fig)
    return all_figs
