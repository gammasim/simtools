...
The generator supports either one locally pulled SIF image for all jobs or a mapping from image
labels to SIF paths. Label-to-image mappings are useful when one job grid is split across several
production images.
...
inside the container. For production images, `SIMTOOLS_SIM_TELARRAY_PATH` is usually set to
`/workdir/sim_telarray`.
