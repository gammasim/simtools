...
This application estimates the total number of thrown Monte Carlo events required for
a production simulation to achieve one of the following targets:
...
Events can be re-weighted to a different power-law spectrum using the `spectral_index` argument.
