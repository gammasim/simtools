...
Each method or function should have a well-defined, focused, fast, and
readable unit test.
...
Coverage is a diagnostic tool; prioritize semantic coverage of branches and
edge cases.
...
Open the matching file in `cov_annotate/`. Lines starting with `!` are not
covered and should drive the next tests you add.

## Workflow

1. Find the production module in `src/simtools/` and the mirrored test file
   in `tests/unit_tests/`.
