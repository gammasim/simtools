...
1. Writing or updating user documentation in `docs/source/user-guide/`.
2. Adding API reference entries in `docs/source/api-reference/`.
3. Creating documentation for new CLI applications in
   `docs/source/user-guide/applications/`.
4. Improving NumPy-style docstrings in library code under `src/simtools/`.
5. Adding changelog fragments in `docs/changes/` for pull requests.

## Core requirements

1. Public functions, classes, and methods must have docstrings.
2. Use NumPy-style docstrings with clear sections:
   - `Parameters`
   - `Returns`
   - `Raises`
   - `Examples`
...
2. Add an entry in `docs/source/user-guide/applications.md` in alphabetical
   order.
3. Ensure the page uses one `.. automodule::` directive matching existing
   application pages.
...
1. Add exactly one line to `docs/changes/<pr-number>.<type>.md`.
