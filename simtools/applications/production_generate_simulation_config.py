...
file_path (str, required)
    Path to the dl2_mc_events_file file used for statistical uncertainty evaluation.
...
def _parse(label):
    ...
    config.parser.add_argument(
        "--file_path", type=str, required=True, help="Path to the dl2_mc_events_file file."
    )
    config.parser.add_argument(
        "--file_type",
        type=str,
        required=True,
        help="Type of the dl2_mc_events_file file ('On-source' or 'Offset').",
    )
    config.parser.add_argument(
        "--metrics_file",
        required=True,
